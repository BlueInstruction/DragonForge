...
TEXTURE_PATCHES: List[PT] = [
    (mk_asgn('options8.UnalignedBlockTexturesSupported'), r'\g<1>TRUE;', 'tx0'),
]

MISC_PATCHES: List[PT] = [
    (mk_asgn('options15.TriangleFanSupported'), r'\g<1>TRUE;', 'rn4'),
]


class VKD3DCapsPatcher:
    CAP_FILES = ['device.c']
    EXCLUDE_DIRS = ['tests', 'demos', 'include', '.git']
    VERSION = "3.2.0"

    def __init__(self, profile: str = 'high', gpu: bool = True, dry: bool = False, verbose: bool = False):
    ...
    def _get_patches(self) -> List[List[PT]]:
        base = [SM_PATCHES, WAVE_PATCHES, RESOURCE_PATCHES, SHADER_PATCHES]
        ext = [MESH_PATCHES, RT_PATCHES, SAMPLER_PATCHES, TEXTURE_PATCHES, MISC_PATCHES]
        if self.profile == 'p3':
            return base
        if self.profile in ['p7', 'p9']:
            return base + ext
        if self.profile == 'high':
            return base + ext + [GPU_PATCHES]
        return base
    ...
    def _find_files(self, src: str, pat: str) -> List[str]:
        ...
        return [f for f in files if not any(ex in f for ex in self.EXCLUDE_DIRS)]
...
def main() -> int:
    import argparse
    parser = argparse.ArgumentParser(description='VKD3D-Proton capabilities patcher')
    parser.add_argument('src', help='Source directory')
    parser.add_argument('--profile', choices=['p3', 'p7', 'p9', 'high'], default='high')
    parser.add_argument('--no-gpu', action='store_true', help='Disable GPU spoofing')
    parser.add_argument('--dry-run', action='store_true', help='Dry run mode')
    parser.add_argument('--verbose', '-v', action='store_true', help='Verbose output')
    parser.add_argument('--report', action='store_true', help='Generate report')
    args = parser.parse_args()

    patcher = VKD3DCapsPatcher(
